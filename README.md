...
**type contract** is validated against **signalk-container > 1.23.0** (the release
that publishes the `signalk-container/types` entrypoint); this is a dev-only check
and imposes no dependency on your plugin.
...
| Types | Local mirror of signalk-container's public API — `ContainerManagerApi`, `ContainerConfig`, `EnsureRunningOptions`, `UpdateServiceApi`, … — verified at build time against `signalk-container/types` (> 1.23.0) so it never silently drifts. Feature-detected members stay optional here. |
