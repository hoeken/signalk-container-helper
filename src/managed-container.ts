...
export interface RouterLike {
  get(path: string, handler: (req: unknown, res: ResponseLike) => unknown): unknown
  post(path: string, handler: (req: unknown, res: ResponseLike) => unknown): unknown
}
...
export interface UpdateRoutesOptions {
   ...
   */
  onApplied?: (requestedTag: string, resolvedTag: string) => void | Promise<void>
}
...
export class ManagedContainer {
  ...
  private resolveTag(requested: string): string {
    if (!isValidImageTag(requested)) {
      this.fail('invalid-tag', `Invalid image tag: ${JSON.stringify(requested)}`)
    }
    const resolved = this.options.resolveTag?.(requested) ?? requested
    if (!isValidImageTag(resolved)) {
      this.fail('invalid-tag', `Invalid resolved image tag: ${JSON.stringify(resolved)}`)
    }
    return resolved
  }
  ...
  async start(tag?: string): Promise<StartResult> {
    ...
    const { manager, runtime } = await waitForContainerManager({
      ...
      onWaiting: (phase) => {
        if (phase === 'runtime') this.status('Waiting for container runtime detection...')
      }
    })
    if (!manager) {
      this.fail(
    ...
    const resolved = this.resolveTag(tag ?? this.options.defaultTag ?? 'latest')
    ...
    if (typeof manager.recreate === 'function') {
      try {
        ...
        this.app.debug(`self-heal probe failed (non-fatal): ${errMsg(probeErr)}`)
      }
    }
  ...
  private registerUpdates(manager: ContainerManagerApi): void {
    ...
    try {
      manager.updates.register({
        ...
        versionSource: buildVersionSource(manager.updates, updates.versionSource)
      })
  ...
  private async awaitReadiness(tag: string): Promise<string> {
    ...
    try {
      ...
      this.fail('not-ready', `${this.options.image}:${tag} started but ${url} never became ready: ${errMsg(err)}`)
    }
    return base
  }
  ...
  async resolveAddress(port: number): Promise<string | null> {
    ...
    try {
      const answer = await manager.resolveContainerAddress(this.options.name, port)
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      this.app.debug(`container stop failed (may already be stopped): ${errMsg(err)}`)
    }
  }
  ...
  async getInfo(): Promise<{ state: ContainerState | 'unknown'; image: string }> {
    const manager = this.manager ?? getContainerManager()
  ...
  async getLogs(options?: { tail?: number; since?: number }): Promise<string[] | null> {
    const manager = this.manager ?? getContainerManager()
   ...
   */
  registerUpdateRoutes(router: RouterLike, options: UpdateRoutesOptions = {}): void {
    ...
    router.post(`${basePath}/apply`, async (req, res) => {
      ...
      const requested =
        typeof body?.tag === 'string' && body.tag.length > 0
          ...
          : this.lastStartedTag ?? this.options.defaultTag ?? 'latest'
      if (!isValidImageTag(requested)) {
        res.status(400).json({ error: `Invalid tag: ${JSON.stringify(requested)}` })
        return
      }
      try {
