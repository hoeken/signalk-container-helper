...
function makeAdopted(overrides: Partial<ConstructorParameters<typeof AdoptedContainer>[0]> = {}) {
  const app = makeApp()
...
describe('AdoptedContainer.unregister', () => {
  ...
  it('swallows unregister errors', async () => {
    ...
    expect(app.debug).toHaveBeenCalledWith(expect.stringContaining('unregister failed'))
  })
})
