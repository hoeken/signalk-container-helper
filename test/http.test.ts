...
describe('waitForHttpReady', () => {
  it('resolves on first success', async () => {
    ...
    await waitForHttpReady('http://x/health', { fetchImpl, maxMs: 200, intervalMs: 5 })
    expect(fetchImpl).toHaveBeenCalledTimes(1)
  })

  it('retries failures until success', async () => {
    const fetchImpl = flakyFetch(2)
    await waitForHttpReady('http://x/health', { fetchImpl, maxMs: 2_000, intervalMs: 5 })
  ...
  it('treats non-2xx as not ready', async () => {
    ...
    await expect(
      waitForHttpReady('http://x/health', { fetchImpl, maxMs: 30, intervalMs: 5 })
...
describe('probeHttpHealth', () => {
  it('returns reachable on a fast success', async () => {
    const result = await probeHttpHealth('http://x/health', { fetchImpl: okFetch() })
  ...
  it('flags a slow success with the elapsed time', async () => {
    ...
    const result = await probeHttpHealth('http://x/health', { fetchImpl, slowMs: 5 })
    expect(result.reachable).toBe(true)
